...
# Create credential directories on host if they don't exist
# These are bind-mounted into the container for credential persistence across rebuilds
mkdir -p ~/.config ~/.claude ~/.codex
chmod 700 ~/.claude ~/.codex 2>/dev/null || true
